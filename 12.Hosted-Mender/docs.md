...
- *(tenantadm)* Stop removal of tenant if delete operation is forbidden

### 🧪 Testing

- *(gui)* Let tests account for longer drawer transitions after mui update
- *(gui)* Worked around slower select overlay removal after mui update

### ⚙️ Miscellaneous Tasks

- Bump the backend-docker-dependencies group across 9 directories with 2 updates
- Bump the backend-golang-dependencies group across 1 directory with 13 updates
...
- *(gui)* Aligned cookie dep location after dependency update
- Removed copyright year from email footers
- Bump the mui group in /frontend with 2 updates
- *(gui)* Updated snapshots after mui update
- Bump the production-dependencies group
- Bump the e2e-test-dependencies group
- Bump the mui group across 1 directory with 3 updates
- *(gui)* Updated snapshots after mui update
- Bump the production-dependencies group
- *(gui)* Aligned licenses reference w/ updated bigcalendar
- *(create-artifact-worker)* Upgrade mender-artifact to v4.4.0
- Slim down dependency installs on lint tests
- Bump the playwright group across 1 directory with 3 updates
- Bump the e2e-test-dependencies group across 1 directory with 4 updates
- Bump the development-dependencies group across 1 directory with 20 updates
- *(gui)* Reverted rspack + msw to versions compatible w/ remainder of the setup
- Bump the mui group across 1 directory with 3 updates
- *(gui)* Adapt to MUI v9.0.1 autocomplete behavior changes
- *(gui)* Removed duplicate role deletion confirmation closure
- *(gui)* Aligned snapshots after mui update
- Use pinned frontend acceptance test images
- Pin spectral to non-broken release
- Release 4.0.3
- *(4.0.x)* Release 4.0.3
